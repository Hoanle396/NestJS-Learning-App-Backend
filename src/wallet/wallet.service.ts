...
export class WalletService {
  ...
  async buyCourse(orderDto:OrderDto,user:User,wallet:number): Promise<any>{
     try{
       const order=new Order()
       const course = await this.courseRep.findOne({where: {id: orderDto.course}})
       ...
       await this.cash.insert({user: user,before:user.money,after:wallet,amount:orderDto.fee,note:"Buy Course"})
